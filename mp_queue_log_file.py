...
class Feeder(Process):
    ...
    def run(self):
        ...
        for nb_trials in self._data:
            self._logger.info("Add to queue: {}".format(nb_trials))
...
def configure_logger():
    handle = TimedRotatingFileHandler("mp_log_test.log")
    ft = "%(asctime)s %(processName)-10s %(name)s %(levelname)-8s "
    ft += "%(message)s"
    log_format = logging.Formatter(ft)
